...
TEST(AuthProcessorTest, CreateUserTest) {
    ...
    std::unique_ptr<kvstore::KVStore> kv(MockCluster::initMetaKV(rootPath.path()));
    {
        ...
        auto retCode = kv->get(kDefaultSpaceId, kDefaultPartId,
                               ...
                               &userVal);
        ASSERT_EQ(ErrorCode::SUCCEEDED, retCode);
    }
    // Test user exists and param 'if_not_exists' == false;
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        ASSERT_EQ(ErrorCode::E_META_USER_EXISTED, resp.get_code());
    }
...
TEST(AuthProcessorTest, AlterUserTest) {
    ...
    // If user not exists
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        ASSERT_EQ(ErrorCode::E_META_USER_NOT_FOUND E_BACKUP_TABLE_FAILED, resp.get_code());
    }
}
...
TEST(AuthProcessorTest, DropUserTest) {
    ...
    // User exists.
    {
        ...
        auto retCode = kv->get(kDefaultSpaceId, kDefaultPartId,
                               MetaServiceUtils::userKey("user1"),
                               &userVal);
        ASSERT_EQ(ErrorCode::E_STORAGE_KVSTORE_KEY_NOT_FOUND, retCode);
    }
}
...
TEST(AuthProcessorTest, GrantRevokeTest) {
    ...
    // grant role to user1 for space, space not exists.
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        ASSERT_EQ(ErrorCode::E_STORAGE_SPACE_NOT_FOUND, resp.get_code());
    }
    ...
    // actual role is GUEST, but revoke role ADMIN, expect error.
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        ASSERT_EQ(ErrorCode::E_META_AUTH_IMPROPER_ROLE, resp.get_code());
    }
    ...
    // role not exists.
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        ASSERT_EQ(ErrorCode::E_META_ROLE_NOT_FOUND, resp.get_code());
    }
    // list users
    {
    ...
    }
    {
        ...
        ASSERT_EQ(ErrorCode::SUCCEEDED, roleRet);
        auto roleCount = 0;
...
TEST(AuthProcessorTest, ChangePasswordTest) {
    ...
    // change password, old password is not valid.
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        ASSERT_EQ(ErrorCode::E_META_AUTH_INVALID_PASSWORD, resp.get_code());
    }
