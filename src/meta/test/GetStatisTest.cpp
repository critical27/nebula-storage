...
struct JobCallBack {
    ...
    folly::Future<nebula::Status> operator()() {
        ...
        req.set_code(ErrorCode::SUCCEEDED);
...
TEST_F(GetStatisTest, StatisJob) {
    ...
    ASSERT_EQ(rc, ErrorCode::SUCCEEDED);

    {
        ...
        ASSERT_NE(ErrorCode::SUCCEEDED, resp.get_code());
    ...
    jobMgr->jobFinished(jobId, cpp2::JobStatus::FINISHED);
    {
        ...
        processor->process(req);
        auto resp = std::move(f).get();
        if (resp.get_code() != ErrorCode::SUCCEEDED) {
    ...
    ASSERT_EQ(rc2, ErrorCode::SUCCEEDED);
    {
        // Job is not executed, job status is QUEUE.
    ...
    // Remove statis data.
    {
        ...
        auto retCode = ErrorCode::SUCCEEDED;
