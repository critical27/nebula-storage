...
TEST(BalanceTest, ExpansionZoneTest) {
    ...
    ASSERT_EQ(ErrorCode::E_META_BALANCER_ALREADY_BALANCED, error(ret));
    {
        std::vector<HostAddr> hosts;
        for (int i = 0; i < 4; i++) {
            hosts.emplace_back(std::to_string(i), i);
...
int32_t verifyBalancePlan(kvstore::KVStore* kv,
    ...
    EXPECT_EQ(retcode, ErrorCode::SUCCEEDED);
    int32_t num = 0;
    while (iter->valid()) {
...
TEST(BalanceTest, SpecifyMultiHostTest) {
    ...
    ASSERT_FALSE(ok(ret));
    EXPECT_EQ(ErrorCode::E_META_BALANCER_NOT_ENOUGH_HOSTS, error(ret));
...
TEST(BalanceTest, StopPlanTest) {
    ...
    sleep(3);
    {
        ...
        std::unique_ptr<kvstore::KVIterator> iter;
        auto retcode = kv->prefix(kDefaultSpaceId, kDefaultPartId, prefix, &iter);
        ASSERT_EQ(retcode, ErrorCode::SUCCEEDED);
        int32_t taskEnded = 0;
