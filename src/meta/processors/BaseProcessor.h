...
#define CHECK_SPACE_ID_AND_RETURN(spaceID) \
    ...
    if (retSpace != ErrorCode::SUCCEEDED) { \
...
#define CHECK_SEGMENT(segment) \
    if (!MetaCommon::checkSegment(segment)) { \
        handleErrorCode(ErrorCode::E_STORAGE_KV_SEGMENT_ILLEGAL); \
        onFinished(); \
        return; \
    }
...
protected:
    ...
    void handleErrorCode(ErrorCode code,
        ...
        resp_.status_ref()->set_code(code);
    ...
    void handleLeaderChanged(GraphSpaceID spaceId, PartitionID partId) {
        ...
        if (ok(leaderRet)) {
            resp_.status_ref()->set_leader(toThriftHost(nebula::value(leaderRet)));
        } else {
            resp_.status_ref()->set_code(nebula::error(leaderRet));
        }
    }
     ...
     * Get all hosts
     * */
    ErrorOr<ErrorCode, std::vector<HostAddr>> allHosts();

    /**
     * Get one auto-increment Id.
     * */
    ErrorOr<ErrorCode, int32_t> autoIncrementId();

    /**
     * Check spaceId exist or not.
     * */
    ErrorCode spaceExist(GraphSpaceID spaceId);

    /**
     * Check user exist or not.
     **/
    ErrorCode userExist(const std::string& account);

    /**
     * Check host has been registered or not.
     * */
    ErrorCode hostExist(const std::string& hostKey);

    /**
     * Return the spaceId for name.
     * */
    ErrorOr<ErrorCode, GraphSpaceID> getSpaceId(const std::string& name);
    ...
    ErrorCode doSyncPut(std::vector<kvstore::KV> data);
    ...
    ErrorOr<ErrorCode, GroupID> getGroupId(const std::string& groupName);

    ErrorOr<ErrorCode, ZoneID> getZoneId(const std::string& zoneName);
