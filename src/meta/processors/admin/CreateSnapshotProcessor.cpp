...
void CreateSnapshotProcessor::process(const cpp2::CreateSnapshotReq&) {
    ...
    if (signRet != ErrorCode::SUCCEEDED) {
    ...
    if (!nebula::ok(meteRet)) {
        LOG(ERROR) << "Create snapshot failed on meta server" << snapshot;
        handleErrorCode(nebula::error(meteRet));
        onFinished();
        return;
    }
...
ErrorCode CreateSnapshotProcessor::cancelWriteBlocking() {
