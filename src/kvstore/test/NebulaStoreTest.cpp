...
TEST(NebulaStoreTest, SimpleTest) {
    ...
    store->asyncMultiPut(0, 0, {{"key", "val"}}, [](ErrorCode code) {
        EXPECT_EQ(ErrorCode::E_STORAGE_SPACE_NOT_FOUND, code);
    });

    store->asyncMultiPut(1, 6, {{"key", "val"}}, [](ErrorCode code) {
        EXPECT_EQ(ErrorCode::E_STORAGE_PART_NOT_FOUND, code);
    });
    ...
    store->asyncMultiPut(1, 1, std::move(data), [&] (ErrorCode code) {
        EXPECT_EQ(ErrorCode::SUCCEEDED, code);
        baton.post();
    });
    baton.wait();
    int32_t start = 0;
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, store->range(1, 1, s, e, &iter));
    int num = 0;
    ...
    for (int part = 0; part < 3; part++) {
        ...
        sleep(FLAGS_raft_heartbeat_interval_secs);
        {
            ...
            for (int i = 0; i < replicas; i++) {
                ...
                std::unique_ptr<KVIterator> iter;
                ASSERT_EQ(ErrorCode::SUCCEEDED, engine->range(s, e, &iter));
                int num = 0;
                auto prefixLen = prefix.size();
                while (iter->valid()) {
                    auto key = *reinterpret_cast<const int32_t*>(iter->key().data() + prefixLen);
        ...
        auto followerIndex = (index + 1) % replicas;
        {
            ...
            stores[followerIndex]->asyncMultiPut(0,
                                                 ...
                                                 {{"key", "val"}},
                                                 [&baton](ErrorCode code) {
                EXPECT_EQ(ErrorCode::E_LEADER_CHANGED, code);
                baton.post();
            });
            baton.wait();
        }
        // Let's try to read data on follower
        {
            ...
            EXPECT_EQ(ErrorCode::E_LEADER_CHANGED, ret);
        }
    }
}
...
TEST(NebulaStoreTest, TransLeaderTest) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        part->asyncTransferLeader(targetAddr, [&] (ErrorCode) {
            baton.post();
        });
        baton.wait();
    }
...
TEST(NebulaStoreTest, AtomicOpBatchTest) {
    ...
    // put kv
    {
        ...
        EXPECT_EQ(ErrorCode::SUCCEEDED, ret);
        while (iter->valid()) {
