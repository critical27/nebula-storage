...
TEST(RocksEngineTest, SimpleTest) {
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->put("key", "val"));
    std::string val;
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->get("key", &val));
    EXPECT_EQ("val", val);
}
...
TEST(RocksEngineTest, RangeTest) {
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->multiPut(std::move(data)));
    ...
    auto checkRange = [&](int32_t start,
        ...
        EXPECT_EQ(ErrorCode::SUCCEEDED, engine->range(s, e, &iter));
        int num = 0;
        while (iter->valid()) {
            num++;
...
TEST(RocksEngineTest, PrefixTest) {
    ...
    auto checkPrefix = [&](const std::string& prefix,
        ...
        std::unique_ptr<KVIterator> iter;
        EXPECT_EQ(ErrorCode::SUCCEEDED, engine->prefix(prefix, &iter));
        int num = 0;
        while (iter->valid()) {
            num++;
            auto key = iter->key();
...
TEST(RocksEngineTest, RemoveTest) {
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->get("key", &val));
    EXPECT_EQ("val", val);
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->remove("key"));
    EXPECT_EQ(ErrorCode::E_STORAGE_KVSTORE_KEY_NOT_FOUND, engine->get("key", &val));
}
...
TEST(RocksEngineTest, RemoveRangeTest) {
    ...
    for (int32_t i = 0; i < 100; i++) {
        ...
        EXPECT_EQ(ErrorCode::SUCCEEDED, engine->put(key, value));
        std::string val;
        EXPECT_EQ(ErrorCode::SUCCEEDED, engine->get(key, &val));
        EXPECT_EQ(value, val);
    }
    ...
    }
    {
        ...
        EXPECT_EQ(ErrorCode::SUCCEEDED, engine->range(start, end, &iter));
        int num = 0;
        int expectedFrom = 50;
        while (iter->valid()) {
            num++;
...
TEST(RocksEngineTest, OptionTest) {
    ...
    EXPECT_EQ(ErrorCode::E_STORAGE_ROCKSDB_SET_OPTION_FAILED,
...
TEST(RocksEngineTest, CompactTest) {
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->multiPut(std::move(data)));
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->compact());
}
...
TEST(RocksEngineTest, IngestTest) {
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->ingest(files));

    std::string result;
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->get("key", &result));
    EXPECT_EQ("value", result);
    EXPECT_EQ(ErrorCode::SUCCEEDED, engine->get("key_empty", &result));
    EXPECT_EQ("", result);
    EXPECT_EQ(ErrorCode::E_STORAGE_KVSTORE_KEY_NOT_FOUND, engine->get("key_not_exist", &result));
}
...
TEST(RocksEngineTest, BackupRestoreTable) {
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, restore_engine->ingest(sst_files));

    std::unique_ptr<KVIterator> iter;
    EXPECT_EQ(ErrorCode::SUCCEEDED, restore_engine->prefix(partPrefix, &iter));
    int index = 0;
    while (iter->valid()) {
        auto key = iter->key();
        auto val = iter->val();
    ...
    EXPECT_EQ(ErrorCode::SUCCEEDED, restore_engine->prefix(tagsPrefix, &iter));
    index = 1;
    int num = 0;
    while (iter->valid()) {
        auto key = iter->key();
