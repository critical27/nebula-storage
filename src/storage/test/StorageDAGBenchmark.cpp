...
public:
    folly::Future<ErrorCode> execute(PartitionID, const T&) {
        VLOG(1) << name_;
...
private:
    std::string name_;
    folly::SharedPromise<ErrorCode> promise_;
...
class FutureDAG {
public:
    folly::Future<ErrorCode> go(PartitionID partId, const T& vId) {
        ...
        for (size_t i = 0; i < nodes_.size() - 1; i++) {
            ...
            std::vector<folly::Future<ErrorCode>> futures;
                ...
                .thenTry([node, partId, &vId] (auto&& t) {
                    ...
                    for (const auto& codeTry : t.value()) {
                        if (codeTry.hasException()) {
                            ...
                            return;
                        } else if (codeTry.value() != ErrorCode::SUCCEEDED) {
        ...
        getNode(inputIdx_)->promise_.setValue(ErrorCode::SUCCEEDED);
        return getNode(outputIdx_)->promise_.getFuture()
            .thenTry([this](auto&& t) -> folly::Future<ErrorCode> {
                CHECK(!t.hasException());
                reset();
                return t.value();
            });
...
private:
    ...
    void reset() {
        for (auto& node : nodes_) {
            node->promise_ = folly::SharedPromise<ErrorCode>();
        }
    }
